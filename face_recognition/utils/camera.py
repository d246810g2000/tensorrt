...
USB webcam or CSI camera.  In addition, this Camera class is further 
extended to take a video file or an image file as input.
"""
...
def add_camera_args(parser):
    ...
    parser.add_argument('--csi', type=int, default=None,
                        help='CSI camera [None]')
...
class Camera():
    ...
    def _open(self):
        ...
        if a.image:
            ...
            self._start()
        elif a.csi is not None:
            logging.info('Camera: using CSI camera')
            self.cap = open_cam_csi(a.width, a.height)
            self._start()
        else:
